import sys
sys.path.append('/home/liamli4465/randomNAS_release')
import os
import shutil
import logging
import inspect
...
class Random_NAS:
    ...
    def get_eval_arch(self, rounds=None):
        ...
        for r in range(n_rounds):
            sample_vals = []
            for _ in range(1000):
                arch = self.model.sample_arch(test=True)
                try:
...
def main(args):
    ...
    if args.benchmark=='ptb':
        ...
        from benchmarks.cnn.darts.darts_wrapper_discrete_proxyless import DartsWrapper
        model = DartsWrapper(save_dir, args.seed, args.batch_size, args.grad_clip, args.epochs, init_channels=args.init_channels, layers=args.layers)
    ...
    if not args.eval_only:
        ...
        archs = searcher.get_eval_arch(11)
    logging.info(archs)
